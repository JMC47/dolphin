...
OpArg VertexLoaderX64::GetVertexAddr(int array, u64 attribute)
{
	...
	if (attribute & MASK_INDEXED)
	{
		...
		MOVZX(64, 32, scratch2, M(&g_main_cp_state.array_strides[array]));
		IMUL(64, scratch1, R(scratch2));
...
int VertexLoaderX64::ReadVertex(OpArg data, u64 attribute, int format, int count_in, int count_out, u8 scaling_exponent, AttributeFormat* native_format)
{
	...
	int store_bytes = sizeof(float) * count_out;
	if (load_bytes > 8)
		MOVDQU(coords, data);
	...
	m_dst_ofs += store_bytes;
	return load_bytes;
}

// TODO: rewrite fallbacks without swapping
...
void VertexLoaderX64::GenerateVertexLoader()
{
	...
	if (m_VtxDesc.Normal)
	{
		...
		if (m_VtxDesc.Normal & MASK_INDEXED && m_VtxAttr.NormalElements && m_VtxAttr.NormalIndex3)
		{
			for (int i = 0; i < (m_VtxAttr.NormalIndex3 ? 3 : 1); i++)
			{
				data = GetVertexAddr(ARRAY_NORMAL, m_VtxDesc.Normal);
				ReadVertex(data, m_VtxDesc.Normal, m_VtxAttr.NormalFormat, 3, 3, scaling_exponent, &m_native_vtx_decl.normals[i]);
			}
		}
		else
		{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		if (tc[i])
		{
			...
			ReadVertex(data, tc[i], m_VtxAttr.texCoord[i].Format, elements, elements, scaling_exponent, &m_native_vtx_decl.texcoords[i]);
